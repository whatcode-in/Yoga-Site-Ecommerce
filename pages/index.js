        ...
        <div className="container">
          <div className="search-field-title-container">
            <p className="text-3xl">
              Find the best, organic and cleansing products for you at the MAVA
              Store
            </p>
              ...
              <div className="flex flex-wrap -m-4 justify-around">
                ...
                {search.length > 0 &&
                  search.slice(0, 3).map((item) => {
                    return (
                      <div
                        className="lg:w-1/5 md:w-1/2 p-4 hover:cursor-pointer w-full m-12 border-2 rounded-md shadow-lg"
                        key={item._id}
                      >
                        <Link
                          key={item._id}
            ...
            {products.length === 0 ? (
              ...
              products.map((item) => {
                return (
                  <div
                    key={item._id}
                    className="lg:w-1/4 md:w-1/2 p-4 hover:cursor-pointer w-full m-12 border-2 rounded-xl shadow-lg"
                  >
                    <Link
                      key={item._id}
                      passHref={true}
                        ...
                        <div className="flex flex-row space-x-2 items-center justify-center">
                          {cart[item._id] != null ? (
                            <>
                              ...
                              <AiFillMinusCircle
                                onClick={() =>
                                  removeFromCart({ itemcode: item._id, qty: 1 })
                                }
                              ...
                              <AiFillPlusCircle
                                onClick={() =>
                                  addToCart({ itemcode: item._id, qty: 1 })
                                }
                                className="text-green-700 cursor-pointer"
                              />
                            </>
                            ...
                            <button
                              className="flex ml-4 text-white bg-green-500 border-0 text-sm sm:text-base py-2 px-2 md:px-6 focus:outline-none hover:bg-green-600 rounded"
                              onClick={() => {
                                addToCart({
                                  itemcode: item._id,
